...
async function updateUserPaymentStatus(
  ...
  try {
    console.log(
      `Updating payment status for user ${userId} with payment method ${paymentMethod}`
    );
...
async function updateSubscriptionStatus(
  ...
  try {
    ...
    console.log(
      `Successfully set subscription status to ${status} for user ${userId}`
    );
    return true;
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Using webhook secret: ${webhookSecret ? "Provided" : "Missing"}`
      );
      ...
      if (event.type.startsWith("customer.subscription.")) {
        ...
        if (newUserStatus) {
          console.log(
            `Need to set user status to: ${newUserStatus} (Reason: ${statusReason})`
          );
          ...
          try {
            ...
            for (const doc of allUsersSnapshot.docs) {
              ...
              console.log(
                `Checking user ${doc.id}: ${userData.email}, stripeCustomerId: ${userData.stripeCustomerId}`
              );
              ...
              if (
                userData.stripeCustomerId === "direct-payment" &&
                customerEmail &&
                userData.email &&
                userData.email.toLowerCase() === customerEmail.toLowerCase()
              ) {
            ...
            if (matchedUserId) {
              console.log(
                `Found matching user: ${matchedUserId} (Reason: ${matchReason})`
              );
              ...
              if (success) {
                console.log(
                  `Successfully updated subscription status to ${newUserStatus} for user ${matchedUserId}`
                );
              } else {
                console.error(
                  `Failed to update subscription status for user ${matchedUserId}`
                );
              }
            } else {
              console.error(
                `Could not find any matching user for this subscription event`
              );
            }
          } catch (error) {
      ...
      if (event.type === "payment_intent.succeeded") {
        ...
        console.log(
          `Payment intent succeeded. Email: ${email}, UID from metadata: ${uid}`
        );
        console.log(
